...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.app','data' => ['title' => 'Profile Settings | TheOAsis Research Supervision System']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Profile Settings | TheOAsis Research Supervision System']); ?>
    ...
    <main class="flex-1 min-w-0 space-y-6">
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
        <div class="flex items-start justify-between gap-3 border-b border-slate-100 dark:border-slate-700 pb-4">
          ...
          <a href="<?php echo e(route('student.dashboard')); ?>" class="px-3 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-semibold transition flex items-center gap-1.5">
        ...
        <form method="POST" action="<?php echo e(route('student.profile.update')); ?>" class="space-y-5">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <input type="text" name="full_name" value="<?php echo e(old('full_name', $student->full_name)); ?>" required class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm focus:border-academic-600 focus:outline-none focus:ring-2 focus:ring-academic-600/20 dark:border-slate-700 dark:bg-slate-700 dark:text-white">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Phone</label>
              <input type="text" name="phone" value="<?php echo e(old('phone', $student->user?->phone ?? $student->phone)); ?>" class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm focus:border-academic-600 focus:outline-none focus:ring-2 focus:ring-academic-600/20 dark:border-slate-700 dark:bg-slate-700 dark:text-white">
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <input type="text" value="<?php echo e($student->matric_number); ?>" disabled class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-slate-100 px-4 py-2.5 text-sm text-slate-500 shadow-sm dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-400">
            </div>
            <div>
              ...
              <select name="degree_level" class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm focus:border-academic-600 focus:outline-none focus:ring-2 focus:ring-academic-600/20 dark:border-slate-700 dark:bg-slate-700 dark:text-white">
                <?php $__currentLoopData = ['BSc', 'MSc', 'PhD']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $degree): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <option value="<?php echo e($degree); ?>" <?php echo e(old('degree_level', $student->degree_level ?? 'BSc') === $degree ? 'selected' : ''); ?>><?php echo e($degree); ?></option>
...
<?php endif; ?><?php /**PATH /Users/olasunkanmiarowolo/Documents/OAsis-RS/Archive/oasis-rpm/resources/views/student/profile.blade.php ENDPATH**/ ?>