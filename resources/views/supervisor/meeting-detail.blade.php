...
@section('title', 'Meeting ' . $meeting->log_id)

@section('content')
<div class="container mx-auto px-4 py-8 max-w-3xl">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Meeting #{{ $meeting->meeting_number }}</h1>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
            <div>
                ...
                <p class="font-mono text-sm text-gray-800">{{ $meeting->log_id }}</p>
            </div>
        ...
        <dl class="divide-y divide-gray-100">
            ...
            @foreach ($rows as $label => $value)
                <div class="px-6 py-3 grid grid-cols-1 md:grid-cols-3 gap-2">
                    ...
                    <dd class="text-sm text-gray-800 md:col-span-2 whitespace-pre-line">{{ $value }}</dd>
                </div>
            @endforeach
        </dl>
    </div>
