              ...
              @for($i=1; $i<=8; $i++)
                <div>
                  <div class="flex justify-between text-xs mb-1">
                    <span class="text-slate-600 dark:text-slate-400">Stage {{ $i }}</span>
                    <span class="font-bold text-slate-900 dark:text-white">{{ rand(1, 3) }} students</span>
                  </div>
                  <div class="w-full bg-slate-100 dark:bg-slate-700 h-2 rounded-full overflow-hidden">
                    <div class="bg-academic-600 dark:bg-academic-400 h-full rounded-full" style="width: {{ rand(10, 80) }}%"></div>
                  </div>
                </div>
              @endfor
            </div>
              ...
              @foreach($months as $index => $month)
                <div class="flex-1 flex flex-col items-center gap-1 group cursor-pointer">
                  <div class="relative w-full flex items-end justify-center h-32 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                    <div class="w-6 bg-academic-600 dark:bg-academic-400 rounded-t-md transition-all duration-300 hover:bg-academic-700 dark:hover:bg-academic-500" style="height: {{ $counts[$index] * 15 }}%"></div>
                    <span class="absolute top-1 text-[10px] font-bold text-academic-700 dark:text-academic-300">{{ $counts[$index] }}</span>
                  </div>
                  <span class="text-[10px] font-medium text-slate-500 dark:text-slate-400">{{ $month }}</span>
                </div>
              @endforeach
            </div>
              ...
              @for($i=1; $i<=5; $i++)
                <div class="flex items-center justify-between p-2 rounded-lg bg-slate-50 dark:bg-slate-700/30">
                  <div class="flex items-center gap-2">
                    <span class="text-xs font-bold text-slate-700 dark:text-slate-300">Stage {{ $i }}</span>
                    <span class="text-[10px] text-slate-500 dark:text-slate-400">Chapter {{ $i }}</span>
                  </div>
                  <div class="flex items-center gap-2">
                    <div class="w-24 bg-slate-200 dark:bg-slate-600 h-1.5 rounded-full overflow-hidden">
                      <div class="bg-rose-500 h-full rounded-full" style="width: {{ $i * 15 }}%"></div>
                    </div>
                    <span class="text-xs font-medium text-slate-700 dark:text-slate-300">{{ $i * 3 }} days avg</span>
                  </div>
                </div>
              @endfor
            </div>
