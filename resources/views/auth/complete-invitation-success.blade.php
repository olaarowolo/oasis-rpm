    ...
    <div class="mx-auto flex min-h-screen max-w-3xl items-center justify-center px-6 py-12">
        <div class="w-full rounded-[2rem] border border-white/10 bg-white/10 p-10 text-center shadow-2xl backdrop-blur">
            ...
            <p class="mt-4 text-base leading-7 text-slate-300">
                {{ $user->name }}, your {{ $roleLabel }} account is now active. Return to the login page and sign in with the credentials for your role.
            </p>
            <a href="{{ route('login') }}" class="mt-8 inline-flex items-center justify-center rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-200">
                Go to login
            </a>
        </div>
    </div>
