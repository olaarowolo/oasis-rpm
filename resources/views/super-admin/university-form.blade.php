...
@section('title', $mode === 'edit' ? 'Edit University' : 'Add University')
...
<div class="space-y-6 max-w-5xl">
    <div class="rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-violet-900 p-6 text-white shadow-2xl lg:p-8">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div class="max-w-3xl">
                <a href="{{ route('super-admin.universities') }}" class="inline-flex items-center gap-2 text-sm font-medium text-violet-200 hover:text-white transition-colors">
                    <i class="fa-solid fa-arrow-left"></i>
                    Back to Universities
                </a>
                <h1 class="mt-4 text-3xl font-black tracking-tight sm:text-4xl">{{ $mode === 'edit' ? 'Edit University' : 'Add University' }}</h1>
                <p class="mt-3 max-w-2xl text-sm leading-6 text-slate-300 sm:text-base">{{ $mode === 'edit' ? 'Update the tenant identity, contact routing, and branding metadata used across the platform.' : 'Create a new tenant with the identifiers and contact information needed for provisioning.' }}</p>
            </div>

            <div class="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur-sm">
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Brand Preview</p>
                <div class="mt-3 flex items-center gap-3">
                    <span class="inline-flex h-12 w-12 rounded-2xl border border-white/20 shadow-inner" style="background-color: {{ old('branding_color', $university->branding_color ?: '#3B82F6') }}"></span>
                    <div>
                        <p class="text-sm font-semibold text-white">{{ old('name', $university->name ?: 'New University') }}</p>
                        <p class="text-xs text-slate-300">{{ old('branding_color', $university->branding_color ?: '#3B82F6') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <form action="{{ $mode === 'edit' ? route('super-admin.universities.update', $university) : route('super-admin.universities.store') }}" method="POST" class="space-y-6">
        @csrf
        @if ($mode === 'edit')
            @method('PUT')
        @endif
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800 overflow-hidden">
            ...
            <div class="grid grid-cols-1 gap-6 px-6 py-6 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $university->name) }}" required class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 dark:border-slate-700 dark:bg-slate-700 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">University Code</label>
                    <input type="text" name="code" value="{{ old('code', $university->code) }}" required maxlength="20" class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm uppercase text-slate-900 shadow-sm focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 dark:border-slate-700 dark:bg-slate-700 dark:text-white">
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800 overflow-hidden">
            ...
            <div class="grid grid-cols-1 gap-6 px-6 py-6 lg:grid-cols-[1fr_240px] lg:items-center">
                <div>
                    ...
                    <input type="text" name="branding_color" value="{{ old('branding_color', $university->branding_color ?: '#3B82F6') }}" required placeholder="#3B82F6" class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 dark:border-slate-700 dark:bg-slate-700 dark:text-white">
                </div>
                <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-900/40">
                    ...
                    <div class="mt-4 rounded-2xl p-5 text-white shadow-sm" style="background: linear-gradient(135deg, {{ old('branding_color', $university->branding_color ?: '#3B82F6') }} 0%, #0f172a 100%);">
                        <p class="text-sm font-semibold">{{ old('name', $university->name ?: 'University Name') }}</p>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-2.5 font-semibold text-white hover:bg-violet-700 transition-colors">
                ...
                {{ $mode === 'edit' ? 'Save Changes' : 'Create University' }}
            </button>
            <a href="{{ route('super-admin.universities') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-5 py-2.5 font-semibold text-slate-700 hover:bg-slate-50 transition-colors dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700/50">
