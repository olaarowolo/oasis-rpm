...
<div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid {{ $line }};border-radius:16px;overflow:hidden;">
  ...
  <div style="padding:24px;">
    ...
    <p style="margin:0 0 14px;"><span style="display:inline-block;padding:4px 10px;border-radius:999px;background:#6366f1;color:#fff;font-size:11px;font-weight:700;letter-spacing:.3px;">COMMENT</span></p>
    <p style="margin:0 0 14px;color:{{ $ink }};font-size:14px;line-height:1.6;">{{ $reviewerName ?? 'Your supervisor' }} left a comment on your "{{ $sectionTitle ?? '' }}" section:</p>
    @if(!empty($comment))
    ...
    @if(!empty($url))
      <p style="margin:6px 0 14px;"><a href="{{ $url }}" style="display:inline-block;padding:10px 18px;background:{{ $brandDark }};color:#fff;text-decoration:none;border-radius:10px;font-size:13px;font-weight:700;">View in the portal</a></p>
    @endif
  </div>
