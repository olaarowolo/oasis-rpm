...
<div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid {{ $line }};border-radius:16px;overflow:hidden;">
  ...
  <div style="padding:24px;">
    ...
    <p style="margin:0 0 14px;color:{{ $ink }};font-size:14px;line-height:1.6;">Dear {{ $isSupervisorNote ? 'Dr. Arowolo' : ($studentName ?? 'Student') }},</p>
    <p style="margin:0 0 14px;"><span style="display:inline-block;padding:4px 10px;border-radius:999px;background:{{ $status['color'] }};color:#fff;font-size:11px;font-weight:700;letter-spacing:.3px;">{{ $status['label'] }}</span></p>
    <p style="margin:0 0 14px;color:{{ $ink }};font-size:14px;line-height:1.6;">{{ $status['line'] }}</p>
    <table style="width:100%;border-collapse:collapse;margin:0 0 14px;">
      <tr><td style="padding:6px 12px 6px 0;color:{{ $muted }};font-size:12px;white-space:nowrap;vertical-align:top;">Meeting</td><td style="padding:6px 0;color:{{ $ink }};font-size:13px;">{{ $meetingNumber ?? '' }}</td></tr>
      <tr><td style="padding:6px 12px 6px 0;color:{{ $muted }};font-size:12px;white-space:nowrap;vertical-align:top;">Log ID</td><td style="padding:6px 0;color:{{ $ink }};font-size:13px;">{{ $logId ?? '' }}</td></tr>
    ...
    @if(!empty($feedback))
      <blockquote style="margin:0 0 14px;padding:12px 16px;background:{{ $bg }};border-left:4px solid {{ $brandGold }};border-radius:8px;color:{{ $ink }};font-size:14px;">Supervisor note: {{ $feedback }}</blockquote>
    @endif
    @if(!empty($url))
