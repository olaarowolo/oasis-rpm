...
<aside id="super-admin-sidebar" aria-label="Super Admin Navigation" class="flex h-full w-full flex-shrink-0 flex-col bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 lg:sticky lg:top-0 lg:self-start lg:h-screen lg:w-80 lg:overflow-hidden">
  <div class="border-b border-slate-200 px-4 py-4 dark:border-slate-700">
    <a href="{{ route('super-admin.dashboard') }}" class="flex items-center gap-3" aria-label="Super Admin Dashboard">
    ...
    @foreach ($navigationGroups as $group)
      <section class="rounded-2xl border border-slate-200 bg-slate-50/70 p-3 dark:border-slate-700 dark:bg-slate-800/40">
        <div class="flex items-center justify-between gap-3 px-1 pb-2">
          <div>
            <p class="text-[10px] font-bold uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">{{ $group['label'] }}</p>
            <p class="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">{{ $group['caption'] }}</p>
          </div>
          <span class="rounded-full bg-white px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:bg-slate-900 dark:text-slate-300">{{ $group['badge'] }}</span>
        </div>
        ...
        <div class="space-y-2">
          @foreach ($group['items'] as $item)
            ...
            @endphp
            <a href="{{ $item['route'] }}"
               class="nav-link flex items-start gap-3 rounded-2xl px-3 py-3 transition-colors {{ $isActive ? 'bg-white text-slate-900 shadow-sm dark:bg-slate-900 dark:text-white' : 'text-slate-600 hover:bg-white hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-900/70 dark:hover:text-white' }}"
               aria-current="{{ $isActive ? 'page' : 'false' }}">
              <span class="mt-0.5 inline-flex h-10 w-10 items-center justify-center rounded-2xl {{ $toneClasses[$item['tone']] ?? $toneClasses['slate'] }}">
    ...
    <section class="rounded-2xl border border-dashed border-slate-300 p-3 dark:border-slate-600">
      ...
      <div class="space-y-2">
        @foreach ($quickCreate as $item)
          <a href="{{ $item['route'] }}" class="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-800/70 dark:hover:text-white">
    ...
    <div class="rounded-2xl bg-slate-50 p-3 dark:bg-slate-800/50">
      <div class="flex items-center gap-3">
        <div class="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 to-purple-700 text-sm font-bold text-white">
          {{ Str::upper(($currentUser?->name ?? 'S')[0]) }}
        </div>
        <div class="min-w-0 flex-1">
          <p class="truncate text-sm font-semibold text-slate-900 dark:text-white">{{ $currentUser?->name ?? 'Super Admin' }}</p>
          <p class="truncate text-[10px] text-slate-500 dark:text-slate-400">{{ $currentUser?->email }}</p>
        </div>
