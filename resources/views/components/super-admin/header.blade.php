  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between h-16 lg:h-14 gap-4">
      ...
      <div class="flex-1 min-w-0 lg:max-w-2xl" role="navigation" aria-label="Breadcrumb">
        ...
        @elseif($hasBreadcrumbMarkup)
          {!! $breadcrumbs !!}
        @else
          <nav class="hidden sm:flex items-center gap-1.5" aria-label="Breadcrumb">
            <ol class="flex items-center gap-1.5 text-sm">
              <li>
                <a href="{{ route('super-admin.dashboard') }}" class="flex items-center gap-1.5 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 transition-colors">
              ...
              @if(Route::currentRouteName() !== 'super-admin.dashboard')
                <li class="flex items-center gap-1.5">
                  ...
                  <span class="font-medium text-slate-900 dark:text-white">{{ ucfirst(str_replace(['super-admin.', '.'], ['', ' '], Route::currentRouteName())) }}</span>
                </li>
              @endif
            </ol>
          </nav>
      ...
      <div class="flex items-center gap-2 lg:gap-3">
        ...
        <div class="relative hidden lg:flex">
          ...
          <div id="notifications-dropdown" 
            ...
            <div class="max-h-64 overflow-y-auto p-2 space-y-1">
              <a href="{{ route('super-admin.audit-logs') }}" class="block p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" role="menuitem">
              ...
              <a href="{{ route('super-admin.system-status') }}" class="block p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" role="menuitem">
            ...
            <div class="p-3 border-t border-slate-200 dark:border-slate-700">
              <a href="{{ route('super-admin.audit-logs') }}" class="text-sm font-medium text-violet-600 hover:text-violet-700 dark:text-violet-400 dark:hover:text-violet-300 block text-center">View all notifications</a>
            </div>
          </div>
        </div>
        ...
        <div class="relative">
          <button type="button" 
            ...
            <span class="hidden lg:inline-block font-medium text-slate-900 dark:text-white truncate max-w-[140px]">{{ $currentUser?->name ?? 'Super Admin' }}</span>
          ...
          <div id="user-dropdown" 
               ...
               role="menu" aria-orientation="vertical">
            <div class="p-3 border-b border-slate-200 dark:border-slate-700">
              <p class="font-semibold text-slate-900 dark:text-white truncate">{{ $currentUser?->name ?? 'Super Admin' }}</p>
